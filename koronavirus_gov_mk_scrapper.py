...
class Scrapper:
    ...
    def __init__(self, *, debug=False):
        self.site_url = 'https://koronavirus.gov.mk/stat'
        self.data = []
        self.debug = debug
    ...
    def get_data(self):
        ...
        try:
            self.logger.debug('Opening Chrome webdriver')
            if self.debug:
                driver = self.chrome_driver()
            else:
                driver = self.headless_chrome_driver()
            self.logger.debug('Requesting ' + self.site_url)
            driver.get(self.site_url)
            self.logger.debug('Waiting for 15 seconds for website to load')
            ...
            for request in driver.requests:
                if request.response and 'https://datastudio.google.com/embed/batchedDataV2' in request.path:
                    ...
                    if len(request_body_json['dataRequest']) == 5:
                        self.logger.debug('Found request that contains required data')
                        response_body_json = json.loads(request.response.body.decode('utf-8')[6:])
                        break
        except TimeoutException:
            self.logger.error('Request timed out, check if {} is up.'.format(self.site_url))
    ...
    def scrape(self):
        self.logger.debug('Data gathering started')
        raw_data = self.get_data()
        if not raw_data:
            self.logger.error("Didn't manage to find the data from requests")
            return
    ...
    def write_to_csv_file(self, *, destination='data.csv'):
        self.logger.debug('Writing data to {}'.format(destination))
